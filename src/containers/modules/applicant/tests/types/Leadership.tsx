...
const Leadership = () => {
  ...
  const [{ data, loading, error }] = useAxios('/leadership/tests/start');
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      axiosClient
        .get(`leadership/tests/user/${data.data.id}/next-questions`)
        .then(({ data }) => {
  ...
  const handleSubmit = () => {
    setALoading(true);
    answer({
      url: `leadership/tests/user/${testId}/answer`,
      data: {
      ...
      .then(() => {
        if (nextQuestion.question_left === 1) {
          ...
          axiosClient.get(`leadership/tests/user/${testId}/next-questions`).then(({ data }) => {
            setNextQuestion(data);
            setSelectedValue(0);
            setALoading(false);
          });
  ...
  return (
    <Box className={classes.container} onKeyDown={handleKeyDown}>
      {/* Оля: Если добавить пропс leadership, то после окончания теста появится кнопка Поделиться при нажатии на которую откроется новая модалка с возможностью поделиться через соц сети, пока скрыла */}
      <CompletedTest
        open={finishModal || done}
        setOpen={setFinishModal}
        name={'Тест лидерства'}
        fullReportLink={'/applicant/leadership/report'}
        mbti={false}
      />
