...
const TestItem = ({
  ...
  const answers = options?.map((item) => ({ value: item.id, label: item.text })) || [];
  ...
  return (
    <Box className={classes.root}>
      ...
      {(imgUrlMobile || imgUrlWeb) && (
        <img
          className={group === 'logic' ? classes.imgLogic : classes.img}
          src={isMobile ? imgUrlMobile?.original_url : imgUrlWeb?.original_url}
          alt=""
        />
      )}
      {group === 'logic' ? (
        <Box className={classes.logicRadio}>
          <RadioGroup
            items={answers}
            selectedValue={selectedValue}
            itemClassName={classes.itemClassNameLogic}
            testItemCheck={classes.itemClassNameLogicCheck}
